...
public class OptimizedTridentProcessingTopology {
	...
	public StormTopology build() {
		...
		inputStream = inputStream.each(TaxiFields.BASE_FIELDS,
				new OptimizedAreaLeavingNotifierAndLocationPropagator(dashboardAdress));
		...
		Stream speedStream = inputStream
				...
				.partitionAggregate( // batch-process entries
						...
						new OptimizedCalculateSpeedAndSpeedingNotifier(dashboardAdress),
	...
	public static OptimizedTridentProcessingTopology createWithListeners(BaseFilter speedListener,
			BaseFilter avgSpeedListener, BaseFilter distanceListener) throws Exception {
		return new OptimizedTridentProcessingTopology("taxi", "localhost", 6379, "http://127.0.0.1:3000", speedListener,
				avgSpeedListener, distanceListener);
	}

	public static OptimizedTridentProcessingTopology createWithTopicAndListeners(String topic, BaseFilter speedListener,
			BaseFilter avgSpeedListener, BaseFilter distanceListener) throws Exception {
		return new OptimizedTridentProcessingTopology(topic, "localhost", 6379, "http://127.0.0.1:3000", speedListener,
				avgSpeedListener, distanceListener);
	}
