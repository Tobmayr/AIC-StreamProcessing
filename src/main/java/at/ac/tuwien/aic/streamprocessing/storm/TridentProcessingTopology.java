...
public class TridentProcessingTopology {
	...
	public static StormTopology buildTopology() {
		...
		AverageSpeedStoreMapper storeMapper = new AverageSpeedStoreMapper();
		...
		inputStream.partitionAggregate(taxiFields, new CalculateSpeed(), taxiFieldsWithSpeed).toStream()
				...
				.each(taxiFieldsWithSpeed, new Debug("avgSpeed")).partitionPersist(factory, taxiFieldsWithAvgSpeed,
				new RedisStateUpdater(storeMapper).withExpire(86400000), new Fields());

		inputStream.partitionAggregate(taxiFields, new CalculateDistance(), taxiFieldsWithDistance).toStream()
				.each(taxiFieldsWithDistance, new Debug("distance")).each(taxiFieldsWithDistance, new RedisFunction());
