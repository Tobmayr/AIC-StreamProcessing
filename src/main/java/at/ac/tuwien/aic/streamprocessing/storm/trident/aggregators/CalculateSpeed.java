...
/**
 * The _Calculate Speed_ operator calculates the speed between two successive locations for each taxi, whereas the distance between two locations can be derived
 * by the Haversine formula. This operator represents a stateful operator because it is required to always remember the last location of the taxi to calculate
 * the current speed.
 */
public class CalculateSpeed extends Aggregator<SpeedState> {
    ...
    protected SpeedState compute(SpeedState previous, TridentTuple tuple) {
        ...
        logger.debug("(speed): [taxiId={}, timestamp={}, latitude={}, longitude={}, speed={}]", tuple.getIntegerByField("id"), timestamp, currentLatitude,
                currentLongitude, String.format("%.3f", speed));
